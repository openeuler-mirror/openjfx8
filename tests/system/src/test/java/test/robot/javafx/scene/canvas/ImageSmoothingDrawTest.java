...
public class ImageSmoothingDrawTest {
    ...
    public void getPixelColors() {
        ...
        Util.runAndWait(() -> {
            lastWhitePixelColor = toColor(robot.getPixelColor(lastWhiteX, heightCenter));
            whitePixelColor = toColor(robot.getPixelColor(
                    (int) (lastWhiteX - (imageCanvas.getWidth() / 4)), heightCenter));
        });
    }
