...
void PopupMenuJava::populate()
{
    ...
    for (int i = 0; i < client()->listSize(); i++) {
        ...
        auto backgroundColor = std::make_tuple(style.backgroundColor().toSRGBALossy<uint8_t>().red, style.backgroundColor().toSRGBALossy<uint8_t>().green, style.backgroundColor().toSRGBALossy<uint8_t>().blue, style.backgroundColor().toSRGBALossy<uint8_t>().alpha);
        auto [r1, g1, b1, a1] = backgroundColor;

        //auto [r2, g2, b2, a2] = style.foregroundColor().toSRGBALossy<uint8_t>();
        auto foregroundColor = std::make_tuple(style.foregroundColor().toSRGBALossy<uint8_t>().red, style.foregroundColor().toSRGBALossy<uint8_t>().green, style.foregroundColor().toSRGBALossy<uint8_t>().blue, style.foregroundColor().toSRGBALossy<uint8_t>().alpha);
        auto [r2, g2, b2, a2] = foregroundColor;
